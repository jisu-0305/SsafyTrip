...
const addSchedule = (schedule) => {
  const dayIndex = plans.value.findIndex(
    (plan) => plan.day === currentDay.value
  );
  if (dayIndex !== -1) {
...
const calculateTotalCost = () => {
  return plans.value.reduce((total, plan) => {
    return (
      total +
      plan.schedules.reduce((dayTotal, schedule) => dayTotal + schedule.cost, 0)
    );
  }, 0);
};
...
const savePlans = async () => {
  try {
    ...
    if (response.data.status === "success") {
      alert(
        `일정이 성공적으로 저장되었습니다. (일정 번호: ${
          response.data.message.split("로")[0]
        })`
      );
